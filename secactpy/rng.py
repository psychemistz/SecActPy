...
def derive_subset_inverse_permutation(
    ...
    full_inv_table : ndarray, shape (n_perm, n_full)
        Full inverse permutation table (e.g., from get_cached_inverse_perm_table).
    ...
    >>> common_pos = np.array([0, 5, 100, 500, 1000])  # 5 common genes
    >>> subset_table = derive_subset_inverse_permutation(full_table, common_pos)
    >>> subset_table.shape
    (1000, 5)
    """
...
def get_derived_inverse_perm_table(
    n_full: int,
    ...
    ----------
    n_full : int
        Full signature size (e.g., 7919 for SecAct).
    common_positions : ndarray, shape (n_common,)
        Positions of common genes in the full signature (sorted, 0-indexed).
    ...
    >>> common_pos = np.array([...])  # indices of common genes in signature
